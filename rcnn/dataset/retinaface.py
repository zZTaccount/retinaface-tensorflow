...
class retinaface(IMDB):
    def __init__(self, image_set, root_path, data_path):
        ...
        self.data_path = data_path #data/retinaface


        self._dataset_path = self.data_path
        # eg:/home/xxx/data/retinaface/train/images
        ...
        # print('dataset/retinaface: origin image size', len(self._fp_bbox_map))
    ...
    def gt_roidb(self):
        ...
        for fp in self._fp_bbox_map: #每个fp代表一张image，是长度不等的list，list每个元素为一个bbox
            if self._split=='test': #如果是test集，roidb只存图片路径
            ...
            # lenth = len(self._fp_bbox_map[fp])
            ix = 0 #对一张图中的bbox计数

            # 分解各个标志
            for aline in self._fp_bbox_map[fp]: #每个aline是一个bbox
                ...
                if self._split=='train':
                  ...
                  for li in range(5):
                    ...
                    if landmark[li][0]==-1. and landmark[li][1]==-1.: #missing landmark
                      ...
                      if li==0:
                        landmark_num+=1
                      if landmark[li][2]==0.0:
                        landmark[li][2] = 1.0 #visible
                      else:
                        # print('landmark[2]: ',landmark[li][2]) # ==1
                        landmark[li][2] = 0.0 #invisible
                ...
                ix += 1
            # 分解完一张图片里的所有bbox
            print('dataset/retinaface: fp={}, bbox.size={}, ix={}'.format(fp,boxes.shape[0], ix))
