...
def anchors_cython(int height, int width, int stride, np.ndarray[DTYPE_t, ndim=2] base_anchors):
    ...
    for iw in range(width):
        ...
        for ih in range(height):
            sh = ih * stride
            for k in range(A):
                all_anchors[ih, iw, k, 0] = base_anchors[k, 0] + sw #x1
                all_anchors[ih, iw, k, 1] = base_anchors[k, 1] + sh #y1
                all_anchors[ih, iw, k, 2] = base_anchors[k, 2] + sw #x2
                all_anchors[ih, iw, k, 3] = base_anchors[k, 3] + sh #y2
    return all_anchors