...
def get_crop_batch(roidb):
    ...
    data_list = []
    label_list = []
    imgs, roidb = get_crop_image(roidb) #TODO：非常重要的处理函数，返回处理后的图片和roi标注框
...
class AA:
  def __init__(self, feat_shape):
    ...
    for i in range(len(feat_strides)):
        ...
        base_anchors = generate_anchors(base_size=base_size, ratios=list(ratios), scales=np.array(scales, dtype=np.float32), stride = stride, dense_anchor = config.DENSE_ANCHOR)
        num_anchors = base_anchors.shape[0]
        # print('io/rpn: num_anchor=',num_anchors)
        feat_height, feat_width = feat_shape[i][-2:]
        feat_stride = feat_strides[i]
        feat_infos.append([feat_height, feat_width])
        ...
        all_anchors = anchors_plane(feat_height, feat_width, feat_stride, base_anchors)
        # print('AA: stride{}: anchors_plane shape={}'.format(stride,all_anchors.shape))
        all_anchors = all_anchors.reshape((K * A, 4))
        ...
        # keep only inside anchors
        anchors = all_anchors[inds_inside, :]
        # print('io/rpn AA: stride{}, all_anchor{}'.format(stride, len(inds_inside)))
  ...
  def assign_anchor_fpn(self, gt_label, im_info, landmark=False, prefix='face', select_stride=0):
    ...
    gt_boxes = gt_label['gt_boxes']
    # clean up boxes
    nonneg = np.where(gt_boxes[:, 4] != -1)[0] #找出非负样本
    gt_boxes = gt_boxes[nonneg]
    if landmark:
      gt_landmarks = gt_label['gt_landmarks']
      gt_landmarks = gt_landmarks[nonneg]
    ...
    total_anchors = sum(anchors_num_list)
    # print('total_anchors', anchors.shape[0], len(inds_inside))

    # TODO: notice: label: 1 is positive, 0 is negative, -1 is dont care
    # 20 × 20 × 2 + 40 × 40 × 2 + 80 × 80 × 2 = 16800个
    labels = np.empty((len(inds_inside),), dtype=np.float32) # inds_inside是生成所有的anchor后保留不超出边界的anchor的index，可认为是保留了所有anchor
    labels.fill(-1)
    ...
    if gt_boxes.size > 0:
        ...
        # overlaps (ex, gt)
        # anchors=(16800,4)
        overlaps = bbox_overlaps(anchors.astype(np.float), gt_boxes.astype(np.float)) # 使用了cython加速，返回每个anchor对于每个gt_box的overlap（an,gt）
        # print('io/rpn: overlaps.shape = ',overlaps.shape)  #overlaps.shape=(N*K) N为16800，K为gt_box个数(每张图都不一样)
        argmax_overlaps = overlaps.argmax(axis=1) #(16800,)
        # print('AAAAAAAAAAio/rpn:', argmax_overlaps.shape)
        max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps] #max_overlaps=(16800,)
        gt_argmax_overlaps = overlaps.argmax(axis=0) #(K,)
        gt_max_overlaps = overlaps[gt_argmax_overlaps, np.arange(overlaps.shape[1])] #(K,)
        gt_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[0] #(K,)

        # 将bbox划分给overlap最大的gt，若over<0.3则为负样本
        if not config.TRAIN.RPN_CLOBBER_POSITIVES:
            # assign bg labels first so that positive labels can clobber them
            labels[max_overlaps < config.TRAIN.RPN_NEGATIVE_OVERLAP] = 0
    ...
    bbox_targets = np.zeros((len(inds_inside), bbox_pred_len), dtype=np.float32)
    if gt_boxes.size > 0:
        # print('AAAAAAAAAio/rpn: GT', gt_boxes.shape, gt_boxes[argmax_overlaps, :4].shape) #gt_box=(K, 5), gt_boxes[argmax_overlaps, :4]=(16800,4)
        bbox_targets[:,:] = bbox_transform(anchors, gt_boxes[argmax_overlaps, :]) #这样将16800个anchor相对于其属于的gt的偏移都计算出来了

    bbox_weights = np.zeros((len(inds_inside), bbox_pred_len), dtype=np.float32)
    bbox_weights[labels == 1, 0:4] = 1.0 #TODO 只计算正样本的bbox损失

    if landmark:
      landmark_targets = np.zeros((len(inds_inside), landmark_pred_len), dtype=np.float32)
      landmark_weights = np.zeros((len(inds_inside), landmark_pred_len), dtype=np.float32)
      if landmark_pred_len==10:
        landmark_weights[labels == 1, :] = 1.0 #TODO 只计算正样本的landmark损失
    ...
    if landmark:
      landmark_target_list = list()
      landmark_weight_list = list()
    anchors_num_range = [0] + anchors_num_list #[0, 800, 3200, 12800]
    label = {}
    for i in range(len(feat_strides)):
        stride = feat_strides[i] #[32, 16, 8]
        feat_height, feat_width = feat_infos[i] #[[20, 20], [40, 40], [80, 80]]
        A = A_list[i] #[2, 2, 2]
        _label = labels[sum(anchors_num_range[:i+1]):sum(anchors_num_range[:i+1])+anchors_num_range[i+1]] #将label按照stride划分为三个
        # print('AAAAAAAAAio/rpn: sum(anchors_num_range[:i+1])=',sum(anchors_num_range[:i+1]))
