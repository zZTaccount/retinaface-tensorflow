...
def get_crop_image1(roidb):
    ...
    preprocess image and return processed roidb
    :param roidb: a list of roidb
    :return: list of img as in mxnet format #TODO: 返回BCHW格式的nparray
    roidb add new item['im_info']
    0 --- x (width, second dim of im)
    |
    y (height, first dim of im)

    引入了数据增强的策略，对于图片做不同尺度的缩放，图片的基准尺寸用的是网络的输入大小640，
    首先将输入图片较短的维度缩放成基础尺寸640，在此基础上根据PRE_SCALES = [0.3, 0.45, 0.6, 0.8, 1.0]随机再进行缩放，
    在得到了调整尺寸的原图后，要根据图片的尺寸调整标注位置框和特征点的尺寸，只要将原来的坐标位置×缩放尺度im_scale。
    设置LIMIT=25对每张图片做25次随机裁剪（这样做的目的是每个epoch的时候图片很大概率都做了不一样的随机改变，做到数据增强），
    每次裁剪的大小即为640 × 640 × 3，同时同样需要对bbox和landmark的标注进行调整，
    即减去随机裁剪图片的在原图中的左上角位置left和up，得到标注坐标在crop中的位置。

    """
    #roidb and each roi_rec can not be changed as it will be reused in next epoch
    num_images = len(roidb)
    # print('io/image get_crop_image1 num_images={}\n'.format(num_images))
    processed_ims = []
    processed_roidb = []
    for i in range(num_images):
        roi_rec = roidb[i]
        if 'stream' in roi_rec:
          ...
          assert os.path.exists(roi_rec['image']), '{} does not exist'.format(roi_rec['image'])
          im = cv2.imread(roi_rec['image'])  #TODO:使用imread读入图片

        SIZE = config.SCALES[0][0] #640*640
        PRE_SCALES = [0.6, 0.8, 1.0]
        # PRE_SCALES = [1.0]
        ...
        if _scale>10.0: #avoid im.size<SIZE, never?
          ...
          # print('io/image: get_crop_image1: im_scale=',im_scale)
          im = cv2.resize(im, None, None, fx=im_scale, fy=im_scale, interpolation=cv2.INTER_LINEAR)
        ...
        while retry<LIMIT:
          ...
          if len(valid)>0 or retry==LIMIT-1:
            # print('io/image: get_crop_image1 ,len(valid) = ',len(valid))
            im = im_new
            new_rec['boxes'] = np.array(valid_boxes) #将有效的人脸box保存下来
        ...
        if config.COLOR_MODE>0 and config.COLOR_JITTERING>0.0:
          im = im.astype(np.float32)
          im = color_aug(im, config.COLOR_JITTERING) #图像增强

        #assert np.all(new_rec['landmarks'][:,10]>0.0)
        # global TMP_ID   #TODO：在本文件内，TMP_ID=-1
        ...
        im_tensor = transform(im, config.PIXEL_MEANS, config.PIXEL_STDS, config.PIXEL_SCALE) #BCHW格式的nparray
