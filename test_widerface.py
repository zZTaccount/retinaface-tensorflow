...
def parse_args():
    ...
    # testing
    parser.add_argument('--prefix', help='model to test with', default='./model/bbox_focal/loss=104.76.ckpt-30', type=str)
    ...
    parser.add_argument('--output', help='output folder', default='./focal_new', type=str)
...
def get_boxes(roi, pyramid):
  ...
  print('finish detect one image! boxes.shape={}'.format(boxes.shape))
...
def test(args):
  ...
  detector = RetinaFace(args.prefix, args.gpu, nms=args.nms_thresh, nocrop=args.nocrop, vote=args.bbox_vote)
  imdb = eval(args.dataset)(args.image_set, args.root_path, args.dataset_path)
  roidb = imdb.gt_roidb()
  gt_overlaps = np.zeros(0)
  overall = [0.0, 0.0] #计算所有的图片的recall
  gt_max = np.array( (0.0, 0.0) )
  num_pos = 0
  print('in the test: roidb size', len(roidb))
  ...
  for i in range(len(roidb)):
    ...
    if 'boxes' in roi:
      gt_boxes = roi['boxes'].copy() #roi[boxes]是在读取图片的时候确定的，用来计算的recall不一定准确,因为detect里面做了nms或vote的，框的数量大大减少
      ...
      if boxes.shape[0]>0:
        ...
        print('[%d]'%i, 'recall', recall, 'gt_boxes=%d, pred boxes=%d'%(gt_boxes.shape[0], boxes.shape[0]), 'recall_all:', recall_all, file=sys.stderr)
    else:
